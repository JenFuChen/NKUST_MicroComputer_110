// Character LCD (HD44780U-compatible) functions by CHin-Shiuh Shieh on 2011-04-04
...
clcd_wait();
CLCD_RS=0;CLCD_RW=0;
CLCD_DB=instruction;CLCD_E=1;CLCD_E=0;
}
...
clcd_wait();
CLCD_RS=1;CLCD_RW=0;
CLCD_DB=c;CLCD_E=1;CLCD_E=0;
}

void clcd_put_string(char *s)
{
while(*s!=0){clcd_put_char(*s);s++;}
}

void clcd_put_byte(unsigned char byte_data)
{
clcd_put_char(ASCII[byte_data/16]);clcd_put_char(ASCII[byte_data%16]);
}

void clcd_put_word(unsigned int word_data)
{
clcd_put_byte(word_data/256);clcd_put_byte(word_data%256);
}
